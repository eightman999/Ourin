...
## DevTools Migrator (オプション / Optional)

### Ghidra

- **バージョン**: 11.x 以上推奨（開発環境では 12.0.4 で検証）
- **ライセンス**: Apache License 2.0
- **著作権者**: National Security Agency (NSA)
- **用途**: SSP 互換 Windows DLL/EXE の逆コンパイル・解析（Migrator 機能）
- **リポジトリ**: https://github.com/NationalSecurityAgency/ghidra
- **公式サイト**: https://ghidra-sre.org/

**重要**: Ghidra は Ourin に**同梱されません**。ユーザーが別途インストールした
`analyzeHeadless` を外部プロセスとして呼び出します。通常起動時は Ghidra / Java に依存しません。

`DecompileAll.java`（`Ourin/DevTools/Migrator/Resources/`）は Ghidra のスクリプト API を使用し、
実行時に `GhidraScriptSource.swift` から一時ファイルとして実体化されます。

**セキュリティステータス**: ⚠️ Migrator は未信頼バイナリを扱いますが、解析は Ourin 本体
プロセス外の Ghidra サブプロセスで行われます（参考: 計画「注意点」）。

アプリ内の About → 「ライセンスを表示…」から Ghidra ライセンス文を参照できます。
...
|---|---:|---|---|
| **satoriya-shiori** | Mc172-3 / `a5746694...` | 里々辞書評価器 | BSD-2-Clause |
| **nlohmann/json** | 3.11.0以上 | SwiftとのJSON Lines IPC | MIT |
| **libiconv** | macOS system | CP932/UTF-8変換 | システムライブラリ |
| **CMake** | 3.20以上 | Universal 2ビルド | BSD-3-Clause |

上流ソースは`satori_core/third_party/satoriya-shiori/`へ固定して同梱し、ライセンス本文を保持します。ローカルpatchは`satori_core/PATCHES.md`、tag/commit/treeは`satori_core/UPSTREAM.md`を正本とします。生成済みhelperはアプリへ署名して同梱され、実行時に外部パッケージを必要としません。
...
| 2026-06-26 | 1.1.0 | DevTools Migrator / Ghidra（Apache 2.0）セクション追加 |
| 2026-07-13 | 1.2.0 | 固定版SATORI Core、Universal 2 helper、BSD-2-Clause情報を追加 |
