...
この旧監査節の「パースのみ／プレースホルダーあり」という一覧は現行実装と一致しないため、完了判定には使用しない。現行コードとテストで確認できる範囲は次のとおり。

- ✅ バルーン描画、可変改行、カーソル移動、クリア、画像挿入、文字書式、アンカー装飾、SERIKO、着せ替え、更新、入力ダイアログはランタイム経路を持つ。
