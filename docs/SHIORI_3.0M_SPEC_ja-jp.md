...
1. **`shiori.cache`**
   - 設定値はロード文脈に保持するが、OurinにはSSP型のゴーストキャッシュ機構がないため実行効果はない。

2. **MAKOTO**
   - `descript.txt`の値は保持するが、translator ABIと`sstp.alwaystranslate`の実行経路は未実装。

3. **外部SAORI**
   - 里々上流の内蔵SSUは利用可能。macOS native SAORIおよびWindows DLL代替bridgeは別スライス。
