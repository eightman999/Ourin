...
### SSTP プロトコル
- [SSTP/1.xM 仕様](SSTP_1.xM_SPEC.md) 🟡 **部分互換** - SSTP プロトコルの macOS 差分仕様
  - TCP/HTTP/XPC サーバ実装済み
  - 基本的な SEND/NOTIFY/COMMUNICATE/EXECUTE メソッド対応
  - TCP bind 範囲、raw TCP body、SHIORI `ReferenceN` 応答マッピングに残課題あり
...
### NAR インストール
- [NAR INSTALL/1.0M 仕様](NAR_INSTALL_1.0M_SPEC.md) 🟡 **部分互換** - NAR パッケージインストールの仕様
  - ダブルクリック/D&D インストール対応
  - 基本的な展開とエラー処理実装済み
  - `refreshundeletemask` 区切り、複合インストール、calendar/language系に残課題あり

## ⚙️ システム実装仕様

### FMO（共有メモリ）
- [FMO について](About_FMO.md) 🟡 **部分互換** - プロセス間共有メモリの実装
  - POSIX 共有メモリとセマフォで実装
  - SSP風レコード形式を公開
  - Windows FMO名/HWNDとの直接互換はプラットフォーム差異として未対応

### YAYA システム
- [YAYA Adapter 仕様 1.0M](OURIN_YAYA_ADAPTER_SPEC_1.0M.md) 🟡 **部分互換** - YAYA ゴーストの実行アダプタ
  - ヘルパープロセスとの IPC 実装済み
  - `yaya.txt` および `.dic` ファイルの解析対応
  - SHIORI/3.0M ブリッジ実装済み
  - by-reference、standalone `when`、一部組み込み関数は部分実装/stub
...
- **2026-06-27**: Codex互換性監査を追加し、実装ステータスをSSP/UKADOC互換性基準へ修正
...
Ourin独自ドキュメントはCC BY-SA 4.0で提供されます。第三者資料は元のライセンスを維持します。
