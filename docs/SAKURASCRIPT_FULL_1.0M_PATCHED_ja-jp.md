...
SSP
記述例
\s[0]\i[200]長いアニメーションを再生中。\![anim,pause,200]ここで、一時停止。
...
SSP
記述例
\s[0]\i[200]長いアニメーションを再生中。\![anim,pause,200]ここで、一時停止。\![anim,resume,200]ここで、再び再生開始。
...
SSPNinix
記述例
\![execute,http-post,http://jbbs.shitaraba.net/bbs/read.cgi/computer/44300/1432574039/,BBS=44300&KEY=1432574039&DIR=computer&MESSAGE=テストメッセージ]ゴースト回覧板３ｒｄの、テストスレに「テストメッセージ」と本文に入れて送信する。※したらば掲示板がスパム対策を強化したため現在書き込み不可。
...
SSPNinix
記述例
\![execute,http-post,http://jbbs.shitaraba.net/bbs/read.cgi/computer/44300/1432574039/,--async=書き込みテスト,--param=BBS=44300&KEY=1432574039&DIR=computer&MESSAGE=テストメッセージ]ゴースト回覧板３ｒｄの、テストスレに「テストメッセージ」と本文に入れて送信する。5秒応答がなければタイムアウトする。※したらば掲示板がスパム対策を強化したため現在書き込み不可。
...
動作中・動作後のイベントに関する仕様はOnPingProgress・OnPingCompleteを参照。
...
--host=ホスト名またはIP
--event=イベントまたは識別子……Onではじまる文字列を指定すると(後述のイベントの替わりに)そのイベントが起きる
動作中・動作後のイベントに関する仕様はOnNSLookupComplete・OnNSLookupFailureを参照。
...
\![reload,aigraph]
AIグラフが表示されていた場合に限り、AIグラフをリロードする。getaistate SHIORIリクエストも再度行われる。
...
# Appendix: Ourin(mac) 差分注記（提案, 2025-07-29 21:06 UTC+09:00）

本セクションは OS 統合系タグの **macOS 置換・制約**をまとめます。本文テーブルの解釈は維持し、ここで Ourin 側の実装方針を明確化します。

## OS 統合系タグ（置換）

| タグ例 | Ourin(mac) 置換/制約 |
|---|---|
| `\![open,URL]` | 既定ブラウザ/アプリ起動は **NSWorkspace.shared.open(_:)** 系で処理。`mailto:` 等も URL として委譲。 |
| `\![execute,...]` | アプリ/ファイルは **NSWorkspace**、HTTP 取得等は **URLSession**（サンドボックス制約下）。非対話のシェル直叩きは既定無効。 |
| `\h[...]`（外部リンク） | クリック時は **NSWorkspace.open** か、必要に応じて内部 `\![raise,...]` にフォールバック。 |
| クリップボード系（該当タグ群） | **NSPasteboard.general** を使用。型は UTI/UTType ベースで登録・取得。 |
| 通知/トレイ相当（`\![set,trayballoon]` 等に準拠する表現） | **NSStatusItem** による常駐＋ **UNUserNotificationCenter** で通知。タイムアウト等は **OS 既定**に従う（Best‑Effort）。 |
| 外観ダーク/ライト参照 | **NSApp.effectiveAppearance** を観測して外観切替を反映。 |

### 挙動上の注意
* **パス区切り**は `/` を正規化（相対はゴースト基準）。  
* **ネットワークアクセス**や**アプリ起動**はユーザ同意/権限内でのみ実行。  
* 明確な Windows 固有 UI（通知領域のバルーン秒数固定など）は **意味を緩めて**実装（OS に依存）。

#### 参考（出典）
- Ukadoc（SakuraScript 一覧）。
- Apple: **NSWorkspace**（URL/アプリ起動, Sleep/Wake 通知）, **NSPasteboard**, **UNUserNotificationCenter**, **NSStatusItem/NSStatusBar**, **effectiveAppearance** など。
