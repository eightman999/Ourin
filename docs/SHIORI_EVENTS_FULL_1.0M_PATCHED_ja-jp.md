...
| `OnScreenSaverStart` / `OnScreenSaverEnd` | `DistributedNotificationCenter` の `com.apple.screensaver.didstart/didstop` を監視。画面スリープ/復帰は別イベント `OnDisplayPowerStatus` にする。 |
