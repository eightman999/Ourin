...
> **前提**: 本計画は監査レポート `AUDIT_CLAUDE`（2026-06-14、→ [AUDITS_COMPLETED.md](AUDITS_COMPLETED.md) / [AUDITS_TODO.md](AUDITS_TODO.md) に集約済み）の P0–P3 指摘が commit `8b0acef` / `b80afac` で実装済みであることを確認した**うえで**、なお残るギャップを再調査したもの。動的なゴースト実機検証は当環境では未実施のため、各 `file:行番号` は静的解析時点の位置であり、着手時に再確認すること。
