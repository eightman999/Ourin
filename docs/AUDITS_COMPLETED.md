...
| **SERIKO `collisionex` の geometry 形状判定と animation-only 当たり判定**（2026-08-14） | `Animation/AnimationEngine.swift:75-142,249-392`（rectangle/ellipse/circle/polygon の形状保持、標準・旧形式パーサー、`animation*.collision*` の実行中限定管理）、`Ghost/GhostManager+Surface.swift:636-656`（実行中IDを渡した実マウスイベントの領域解決）。テスト `AnimationEngineTests.swift`（通常時非表示、ellipse/circle、優先順、再読込重複防止）。画像色 `region` は未実装で監査TODOに残す。 |
...
| **SERIKO `collisionex` geometry shape hit testing and animation-only regions** (2026-08-14) | `Animation/AnimationEngine.swift:75-142,249-392` (rectangle/ellipse/circle/polygon shape retention, standard/legacy parsing, and active-only `animation*.collision*` storage), `Ghost/GhostManager+Surface.swift:636-656` (live mouse-event resolution with active IDs). `AnimationEngineTests.swift` covers inactive filtering, ellipse/circle, precedence, and reload deduplication. Image-colour `region` remains in the audit TODO. |
