...
| **SERIKO `collisionex` の circle/polygon 形状判定** | `Animation/AnimationEngine.swift:75-142,324-357`（形状を保持した `CollisionRegion.contains` とパーサー）、`Ghost/GhostManager+Surface.swift:490-498`（実マウスイベントの領域解決）。テスト `AnimationEngineTests.swift`（円形・三角形の登録と内外判定） |
...
| **SERIKO `collisionex` circle/polygon shape hit testing** | `Animation/AnimationEngine.swift:75-142,324-357` (shape-preserving `CollisionRegion.contains` and parser), `Ghost/GhostManager+Surface.swift:490-498` (live mouse-event region resolution). Test `AnimationEngineTests.swift` covers circle/triangle registration and inside/outside hits |
