...
| **`\__v` 音声合成制御と `\_v` 音声アセット再生** | `Ghost/GhostManager.swift:241-246`（音声合成状態）、`2860-2906`（`enable`/`disable`/`alternate` と通常文の既定オフ）、`2619-2627`（`\_v` → `playSound`）、`2992-3000`（`\_V` の再生完了待ち）。通常文を macOS 既定音声へ自動投入せず、音声アセットは `ghost/sound` から独立再生する。 |
