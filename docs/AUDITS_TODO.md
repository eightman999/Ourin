...
| 優先度 | 項目 | 現状・修正案 |
|---|---|---|
| P2 | SERIKO描画メソッド・バルーン右側表示・wordwrap/alignment の細部未検証 | `collisionex` の rectangle/circle/polygon の形状判定は2026-08-12に実装・単体検証済み。残るのは実シェル/バルーンセットでの描画差分テスト。 |
...
| P3 | MAYUNA（着せ替え）の網羅性 | コア経路は実装・単体/ランタイム要求スパイ検証済み（単一/複数タプル、カテゴリ一括、トグル、`bind-noevent`、起動NOTIFY・メニューGET・スクリプト通知、100要素以上の差分省略）。残りは実シェルのオーバーレイ描画、シェル固有のbindgroup制約・相互排他、実ゴーストでの差分確認。 |
...
| Priority | Item | Current State / Fix |
|---|---|---|
| P2 | SERIKO render methods, balloon right display, wordwrap/alignment fine points unverified | `collisionex` rectangle/circle/polygon hit testing is implemented and unit-tested as of 2026-08-12. Remaining work is real shell/balloon-set rendering diff testing. |
...
| P3 | MAYUNA (dressup) thoroughness | Core path is implemented and covered by unit/runtime-request-spy tests (single/repeated tuples, category-wide, toggle, `bind-noevent`, startup NOTIFY, menu GET, script notification ordering, and the 100+ element notification skip). Remaining work is real-shell overlay rendering, shell-specific bindgroup constraints/exclusivity, and in-ghost diff verification. |
...
| **P3** | MAYUNA網羅（実シェル描画・bindgroup制約・実ゴースト差分） |
