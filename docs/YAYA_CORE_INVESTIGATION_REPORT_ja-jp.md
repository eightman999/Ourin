...
> **2026-06-27 ステータス修正:** 本書は2025年時点の履歴文書です。
> 現在の `yaya_core` は IPC のみではなく、C++パーサー/VM、辞書ロード、
...
**履歴上の記述:** 本調査当時、yaya_coreモジュールは基本フレームワークのみ実装で、
YAYA言語インタプリタ機能は未実装でした。現在はこの記述は古く、parser/VM/builtinの実装が進んでいます。
残課題は `yaya_core/IMPLEMENTATION_STATUS.md` に集約されています。
...
#### ❌ 2025年当時の未実装部分（現在は `IMPLEMENTATION_STATUS.md` を参照）
